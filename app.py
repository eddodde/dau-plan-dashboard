...
insight([
    ...
    "병목은 두 가지 — ① <b>발송 시점</b>: 타 패션 플랫폼은 실시간 행동 트리거로 전환(하단 검증 사례)한 반면 당사는 D-1(전일) 기준 발송으로 격차 존재 "
...
with tc2:
    ...
    <li><b>배경</b>: 실시간 행동 트리거는 패션 커머스(29CM·W컨셉·무신사)에서 DAU·재방문 개선이 <b>검증된 방식</b> — 당사는 D-1(전일) 기준 발송으로 격차 존재. 앱푸시 경유 DAU −20%로 개선 여지 최대</li>
...
st.markdown('<div style="font-weight:700;font-size:15px;margin:16px 0 4px">타사 검증 사례 — 패션 커머스 실시간 메시징 전환 (트랙 A 근거)</div>',
            unsafe_allow_html=True)
st.markdown("""
<table class="scen">
<tr><th>플랫폼</th><th>AS-IS → TO-BE</th><th>개선 실적</th></tr>
<tr><td><b>29CM</b></td>
    <td class="l">일괄 대량 발송 → 실시간 인앱 행동 기반 취향 푸시</td>
    <td class="l">WAU +43% · 푸시 CTR 6배 · 인당 체류 7분 30초 이상</td></tr>
<tr><td><b>W컨셉</b></td>
    <td class="l">전일(D-1) 기준 수동 추출 → 행동 기반 이탈 감지·골든타임 내 발송</td>
    <td class="l">이탈 고객 재접속률 +22% · 주간 방문 빈도 1.5회→2.3회 · 구매 전환 +102%</td></tr>
<tr><td><b>무신사</b></td>
    <td class="l">고정 기획전 중심 노출 → 클릭·검색 행동 실시간 반영 초개인화</td>
    <td class="l">월평균 DAU 300~350만 유지 · 인당 월 20회 이상 접속 · 일평균 체류 약 12분</td></tr>
</table>
""", unsafe_allow_html=True)
st.caption("특히 W컨셉은 당사와 동일한 '전일 기준 → 행동 기반 실시간' 전환 사례로 트랙 A의 직접 비교 대상 — "
           "주간 방문 빈도 개선(1.5회→2.3회)은 본 과제의 목표 지표(스티키니스)와 일치. 타사 공개 사례 정리 자료 기반.")
